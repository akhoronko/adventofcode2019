...
def parse_input():
    wires = []
    for w in INPUT:
        steps = tuple((s[0].upper(), int(s[1:])) for s in w.split(","))
        wires.append(steps)
    return wires


def get_paths(w):
    paths_x = defaultdict(list)
    paths_y = defaultdict(list)
    current = (0, 0)
    for direction, step in w:
        x, y = current
        if direction == "U":
            next_point = (x, y + step)
            paths_x[x].append((y, y + step))
        elif direction == "D":
            next_point = (x, y - step)
            paths_x[x].append((y - step, y))
        elif direction == "R":
            next_point = (x + step, y)
            paths_y[y].append((x, x + step))
        elif direction == "L":
            next_point = (x - step, y)
            paths_y[y].append((x - step, x))
        else:
            print(f"bad step: {direction}{step}")
            break

        current = next_point

    return dict(paths_x), dict(paths_y)
...
def get_crossings(paths_x, paths_y):
    # print("get_crossings")
    # print(paths_x)
    # print(paths_y)

    crossings = []
    for x, y_paths in paths_x.items():
        # print(x, y_paths)
        for y, x_paths in paths_y.items():
            if not x and not y:
                continue

            # print(y)
            for start, end in y_paths:
                if y in range(start, end + 1) and any(
                    x in range(s, e + 1) for s, e in x_paths
                ):
...
def calc_steps(w1, w2, cross):
    count = 0
    for w in (w1, w2):
        current = (0, 0)
        for direction, step in w:
            x, y = next_x, next_y = current
            ...
            if next_x == cross[0]:
                start, end = sorted((y, next_y))
                if start <= cross[1] <= end:
                    start, end = sorted((y, cross[1]))
                    count += end - start
                    # print("break", count)
                    break

            if next_y == cross[1]:
                start, end = sorted((x, next_x))
                if start <= cross[0] <= end:
                    start, end = sorted((x, cross[0]))
                    count += end - start
                    # print("break", count)
                    break
...
def run():
    ...
    paths_x1, paths_y1 = get_paths(w1)
    # print(w1, paths_x1, paths_y1)
    paths_x2, paths_y2 = get_paths(w2)
    # print(w2, paths_x2, paths_y2)

    crossings = set(get_crossings(paths_x1, paths_y2))
    crossings.update(get_crossings(paths_x2, paths_y1))

    return min(map(partial(calc_steps, w1, w2), crossings))
